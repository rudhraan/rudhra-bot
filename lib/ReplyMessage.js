const { getContentType, jidNormalizedUser, downloadMediaMessage } = require("@adiwajshing/baileys");
...
class ReplyMessage extends Base {
    ...
    patch(data) {
        ...
        this.isBot = this.id && this.id.startsWith('BAE5') && this.id.length === 16;
   ...
   async download(type = 'file') {
    ...
    return filePath;
       ...
       async downloadMedia(filename) {
    const buffer = await downloadMediaMessage(this.data, "buffer");
    const mediaType = Object.keys(this.data.message)[0];
  const extension = '.' + this.data.message[mediaType].mimetype.split(';')[0].split('/')[1];
