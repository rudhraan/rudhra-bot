...
exports.parseGistUrls = text => {
        ...
        return !matches ? false : matches.filter(url => !url.includes('rudhra-bot')).map(url => `${url}/raw`);
    };
...
const imageKit = new ImageKit({
    publicKey: 'public_8HfVTGQ/RLQOyULs7xv5JE/TCtQ=',
    privateKey: 'private_6YOqzoBDJDisFeQmVxnZ0pBf79c=',
    urlEndpoint: 'https://ik.imagekit.io/princerudh'
});
...
exports.uploadToImageKit = async (buffer, type) => {
    const fileName = `${Date.now()}`;
    const filePath = path.join(TEMP_DIR, fileName);

    try {
              if (type === 'audio') {
            const outputPath = `${filePath}.mp4`;
            return new Promise((resolve, reject) => {
                const tempInputPath = `${filePath}.temp`;
                    ...
                    .on('end', async () => {
                        try {
                            ...
                            const uploadResponse = await imageKit.upload({
                                file: outputMedia.toString('base64'),
...
exports.TelegraphUpload = async (file) => {
    return new Promise((resolve, reject) => {
