...
const {updateMode,getMode} = require("./database/mode");
    ...
    viewOnceMessage: {
      message: {
        messageContextInfo: {
          ...
          contextInfo: quoted // Ensure this is correctly placed here
        },
...
async function igdl(igurl) {
  ...
  try {
    ...
    if (typeof html !== "string") {
      throw new Error("Invalid response: HTML content is not a string");
    }
...
createInteractiveMessage,
 ...
 /*
parsedJid :  function parsedJid(text = "") {
  return [...text.matchAll(/([0-9]{5,16}|0)(@[a-zA-Z.]+)?/g)].map((v) => {
    if (v[2] === "@g.us") {
      return v[0];
    } else if (v[2] === undefined || v[2] === "@whatsapp.net") {
      return v[1] + "@s.whatsapp.net";
    } else {
      return v[0]; // Handles other cases like @example.com without modification
    }
  });
},
*/
    ...
    return new Promise((resolve, reject) => {
      ...
      const bitly = new BitlyClient("dcd9d1d2efcb6b58779ee77cc139c71b09cd1065");
      bitly
        .shorten(url)
        .then((a) => {
          resolve(a);
   ...
   getRandom: function getRandom(input) {
    if (Array.isArray(input) || this instanceof String)
  ...
  convertTextToSound: async (text, lang) => {
  try {
    const options = {
      lang: lang,
      slow: false,
    ...
    songbuffer,
    coverBuffer,
    options = { title: "Rudhra-Bot", artist: ["Prince Rudh"]}
  ) => {
