...
const [pname, pb] = config.STICKER_DATA === 'false' ? [] : config.STICKER_DATA.split(/[,;]/);
...
const addExif = async (imagePath, metadata = {}) => {
  ...
  if (finalMetadata.packname || finalMetadata.author) {
    ...
    const exifData = {
      'sticker-pack-id': "github.com/princerudh/rudhra-bot",
...
exports.song = async (outputPath, videoUrl, bitrate) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            stream.on('error', err => reject(new Error('stream error')));
